...
#setwd("C:/pobreza22/R") # Se puede modificar el directorio si no abre el proyecto
...
fwrite(rezedu, "Bases/ic_rezedu22.csv", row.names=F)
...
fwrite(ocupados, "Bases/ocupados22.csv", row.names=F)
...
fwrite(salud, "Bases/ic_asalud22.csv", row.names=F)
...
fwrite(trab, "Bases/prestaciones22.csv", row.names=F)
...
fwrite(pens, "Bases/pensiones22.csv", row.names=F)
...
fwrite(segsoc, "Bases/ic_segsoc22.csv", row.names=F)
...
fwrite(cev, "Bases/ic_cev22.csv", row.names=F)
...
fwrite(sbv, "Bases/ic_sbv22.csv", row.names=F)
...
fwrite(menores, "Bases/menores22.csv", row.names=F)
...
# nutritiva y de calidad a la población en hogares que presenten, al menos, 
# una de las siguientes características:
#  
...
fwrite(ali, "Bases/ic_ali22.csv", row.names=F)
...
fwrite(trab, "bases/aguinaldo22.csv")
...
ing <- ing %>%
        mutate(ing_1=ifelse(clave=="P008" | clave=="P015", (ing_1/may22)/12, ing_1), 
               ing_1=ifelse(clave=="P009" | clave=="P016", (ing_1/dic21)/12, ing_1),
...
fwrite(ing, "bases/ingreso_deflactado22.csv")
...
fwrite(nomon, "bases/ingresonomonetario_def22.csv")
...
fwrite(esp, "bases/esp_def22.csv")
...
fwrite(reg, "bases/reg_def22.csv")
...
fwrite(ict, "bases/ingresotot22.csv")
...
fwrite(tam_hogesc, "bases/tamhogesc22.csv")
...
fwrite(p_ing, "bases/p_ingresos22.csv")
...
fwrite(pobreza, "Bases/pobreza_22.csv", row.names=F)
...
por <- as.data.frame(matrix(unlist(pob_w %>% 
                                   srvyr::select(vars) %>%
                                   summarise_all(survey_mean, vartype=NULL)), ncol = 1, byrow=T)*100) %>% round(6)
